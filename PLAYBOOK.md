...
| ~~`/blog/`~~ | **No longer a 404, 2026-08-31.** The condition on this row was "revisit only if a notes section ever launches", and one has: `/blog/` is now the real blog index, in `sitemap.xml` and `llms.txt` like any other page. No stub was needed, because the old URL and the new page are the same address |
...
**Week 3 — Add the agent layer everywhere.** Point the auditor at real, live sites (the README shows the one-line change). ~~Ask Claude to add a third agent to your repo — a content writer that drafts a monthly, locally-relevant blog post and files it as a *draft for your approval* rather than publishing (that approval step is your quality moat).~~ **BUILT 2026-08-31** (`agents/content-writer.md`, `.github/workflows/content-writer-agent.yml`), with one change to the plan: it runs **weekly**, not monthly, because the blog started empty. The cadence is a ceiling rather than a quota, and a week with no strong topic files a `content-skipped` issue instead of a thin post. The approval step is exactly as described: it opens a pull request and never merges. If a client is on Wix, connect the [Wix MCP](https://www.wix.com/studio/developers/mcp-server) to Claude and experience "never touching Wix" firsthand. This is also the week to skim what the industry is doing — [agentic SEO workflows](https://www.lyzr.ai/blog/ai-agents-for-seo/) and [current AI SEO agents](https://nightwatch.io/blog/best-ai-seo-agents/) — not to buy anything, but so you can name-drop and compare honestly.
